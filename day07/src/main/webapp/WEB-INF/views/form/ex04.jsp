...
<form method="POST">
	<div id="paramList">
		<p>
			<input name="list[0].name" placeholder="이름">
			<input name="list[0].age" placeholder="나이">
		</p>
	</div>
	<p>
...
<script>
	const addHandler = function(event) {
		const pCount = document.getElementById('paramList').children.length
		console.log(pCount)
		...
		input1.name = 'list[' + pCount + '].name'		// 첫번째 input의 name은 list[i].name
		input1.placeholder = '이름'						// 첫번째 input의 placeholder는 이름
		input2.name = 'list[' + pCount + '].age'		// 두번째 input의 name은 list[i].age
